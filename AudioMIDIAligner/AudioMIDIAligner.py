...
# audio_file = 'E:\\Data\\Audio\\Wavs\\1.wav'
# midi_file = 'E:\\Data\\MIDI\\001-003.mid'


def align_midis(all_songs):
    for song_nr in all_songs:
        align_midi(all_songs[song_nr])
        print(str(song_nr) + ' done!')


def align_midi(song):
    audio_file = song.full_audio_path
    for midi_file in song.full_midi_paths:
        try:
            ...
            midi_audio = midi_object.synthesize(fs=FS)
            ...
            # midi_audio_aligned = midi_object.synthesize(fs=FS)
            ...
            song.add_aligned_midi(midi_object, score)
            midi_object.write(midi_file.replace('MIDI', 'MIDIAligned'))
            print('{0} aligned with score {1}'.format(str(midi_file), str(score)))
        except:
            print(str(midi_file) + ' failed')

# librosa.output.write_wav('C:\\Users\\daphn\\Desktop\\Test1.wav', midi_audio_aligned, FS)
# midi_object.write('C:\\Users\\daphn\\Desktop\\Test3.mid')
# print "Confidence score: {}".format(score)
# Play the result!
# IPython.display.Audio([midi_audio_aligned, audio_data], rate=FS)
