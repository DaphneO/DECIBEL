...
def _dtw(distance_matrix: np.ndarray, gully: float = 1., additive_penalty: float = 0.,
         multiplicative_penalty: float = 1.) -> (np.ndarray, np.ndarray, float):
    """
...
def align_midi(audio_cqt: np.ndarray, audio_times: np.ndarray, full_synthesized_midi_path: str,
    ...
    midi_cqt, midi_times = _compute_cqt(midi_audio)
    ...
    p, q, score = _dtw(distance_matrix, GULLY, penalty)
    ...
    fh.write_chord_alignment_score(midi_name, score)
        ...
        if not fh.file_exists(write_path):
            ...
            try:
                ...
                if not fh.file_exists(synthesized_midi_path):
                    ...
                    synthesizer.synthesize_midi_to_wav(midi_path, 22050)

                if not audio_loaded:
                    # Load audio if it is not loaded yet
                    audio_data, _ = librosa.load(song.full_audio_path, sr=FS)
                    audio_cqt, audio_times = _compute_cqt(audio_data)
                    audio_loaded = True
                align_midi(audio_cqt, audio_times, synthesized_midi_path, write_path)
