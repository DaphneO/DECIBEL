...
def sample(all_songs):
    ...
    :param all_songs: Data set of all songs
    :return: Tuple with training set and test set
    """
    all_songs_keys = []
    for song_key in all_songs:
        if all_songs[song_key].audio_features_path != '':
            all_songs_keys.append(song_key)
    random.seed = 25  # We set the seed to be able to reproduce results
    train_keys = random.sample(all_songs_keys, 100)

    train_songs = dict()
    test_songs = dict()

    for song_key in all_songs_keys:
        if song_key in train_keys:
            train_songs[song_key] = all_songs[song_key]
        else:
            test_songs[song_key] = all_songs[song_key]
...
class HMMParameters:
    def __init__(self, train_songs, chords_list):
        # type: (dict, list) -> None
        """
        ...
        for train_song_key in train_songs:
            ...
            if train_song.audio_features_path != '':
                ...
                for frame_index in range(features.shape[0]):
                    ...
                    chord_index = self._get_index_in_alphabet(Chords.Chord.from_harte_chord_string(
    ...
    def _read_tab_file_path(self, chords_from_tab_file_path):
        # type: (str) -> (int, np.array, np.array, np.array)
        """
        ...
        for i in range(nr_of_chords_in_tab):
            chord_ids[i] = self._get_index_in_alphabet(Chords.Chord.from_harte_chord_string(chords_from_tab[i, 4]))
    ...
    def transpose_chord_label(self, chord_label, nr_semitones_higher):
        # type: (int, int) -> int
        """
    ...
    def jump_alignment(self, chords_from_tab_file_path, audio_features_path, lab_write_path, p_f=0.5, p_b=0.6):
        ...
        for semitone_transpsition in range(12):
            # Transpose
            transposed_chord_ids = \
                np.array([self.transpose_chord_label(c_i, semitone_transpsition) for c_i in chord_ids])
            ...
            if log_likelihood > best_likelihood:
                best_transposition, best_g, best_tr, best_last_chord, best_likelihood = \
                    semitone_transpsition, g, tr, last_chord, log_likelihood

        # Transpose to the best transposition
        transposed_chord_ids = np.array([self.transpose_chord_label(c_i, best_transposition) for c_i in chord_ids])
        ...
        with open(lab_write_path, 'w') as write_file:
            for b in range(len(beat_times) - 2):
                if viterbi_path[b] != last_chord:
                    chord_str = self.chord_label_to_chord_str(viterbi_path[b - 1])
            ...
            if beat_times[len(beat_times) - 2] != beat_start:
                chord_str = self.chord_label_to_chord_str(viterbi_path[len(beat_times) - 2])
